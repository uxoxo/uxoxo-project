...
enum tab_feat : unsigned
{
    ...
    tf_standard   = tf_closable | tf_icon | tf_tooltip,
    tf_all        = tf_closable  | tf_icon    | tf_tooltip
                  | tf_badge     | tf_context | tf_pinnable
                  | tf_color     | tf_renamable
};

constexpr unsigned operator|(tab_feat _a,
                             tab_feat _b) noexcept
{
    return static_cast<unsigned>(_a) | static_cast<unsigned>(_b);
}

constexpr bool has_tf(unsigned _f,
                      tab_feat _bit) noexcept
{
...
constexpr unsigned operator|(tab_control_feat _a,
                             tab_control_feat _b) noexcept
{
    return static_cast<unsigned>(_a) | static_cast<unsigned>(_b);
}

constexpr bool has_tcf(unsigned          _f,
                       tab_control_feat  _bit) noexcept
{
    return (_f & static_cast<unsigned>(_bit)) != 0;
}
